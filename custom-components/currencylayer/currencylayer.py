...
CONF_QUOTE = 'quote'
CONF_NAME = 'name'
DEFAULT_NAME = 'Currency Layer Sensor'
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    rest.update()
    add_devices([CurrencylayerSensor(rest,
...
class CurrencylayerSensor(Entity):
    ...
    def __init__(self, rest, name, base, quote):
...
class CurrencylayerData(object):
    ...
    def update(self):
        ...
        try:
            result = requests.get(self._resource + '?source=' + self._base +
                                  '&access_key=' + self._api_key, timeout=10)
